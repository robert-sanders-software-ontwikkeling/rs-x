...
> implements ISingletonFactory<TId, TData, TInstance, TIdData> {
  ...
  public toLines(indent: number = 4, level: number = 0): string[] {
    ...
    lines.push(
      spaces + '___________________________________________________________',
    );
    ...
    for (const [key, instance] of this._instances.entries()) {
      ...
      lines.push(
        spacesNext + `key (ref. count = ${this._referenceCounts.get(key)}):`,
      );
  ...
  public toString(indent = 4, level = 0): string {
    return this.toLines(indent, level)
      .map((l) => ' '.repeat(indent * level) + l)
      .join('\n');
  }
