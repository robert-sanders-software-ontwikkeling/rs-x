...
): { expressionString: string; type: string } {

    const parent = expression.parent;
    const isLastChild =
        parent?.childExpressions?.at(-1) === expression;
    ...
    return items.filter(item => {
        if (item.expression.type === ExpressionType.Identifier) {
            ...
            if (item.expression.parent?.type === ExpressionType.Member) {
                ...
                return item.expression === parentChildExpressions[parentChildExpressions.length - 1];
            }
        }
        return true;
    })
