...
import { type IModelWithId } from './model-with-id.interface';
...
class PageProxy {
  ...
  private readonly handleMessage: Record<
    MessageType,
  ...
  constructor(
    ...
    this._expressionManager = this._injectionContainer.get(
      RsXExpressionParserInjectionTokens.IExpressionManager,
    );
    this._guidFactory = this._injectionContainer.get(
...
(function () {
  const expressionManager: IExpressionManager =
    window.RSX_INJECTION_CONTAINER?.get(
      RsXExpressionParserInjectionTokens.IExpressionManager,
    );
  ...
  new PageProxy(window.RSX_INJECTION_CONTAINER, expressionManager);
})();
