...
const TestComponent: React.FC<{ model: { x: number; r: number } }> = ({ model }) => {
...
describe('RS-X React E2E', () => {
  it('updates expression result when model changes', async () => {
    ...
    // Initial computation
    expect(resultDiv.textContent).toBe(String(model.r * model.x * (1 - model.x))); // 5 * 3 * (1-3) = -30

    // Update x
    await user.clear(xInput);
    await user.type(xInput, '2');
    expect(resultDiv.textContent).toBe(String(model.r * model.x  * (1 - model.x ))); // 5 * 2 * -1 = -10

    // Update r
    await user.clear(rInput);
    await user.type(rInput, '4');
    expect(resultDiv.textContent).toBe(String(model.r * model.x  * (1 - model.x ))); // 4 * 2 * -1 = -8
  });
});