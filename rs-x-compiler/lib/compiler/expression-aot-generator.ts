...
export function generateAotCompiledExpressionsModule(
  ...
  const code = [
    "import { expandCompactCompiledPlans, registerCompiledExpressionPlansInExpressionCache } from '@rs-x/expression-parser/aot-runtime';",
    '',
    'const compactPlans = [',
    compactEntries,
    '];',
    ...
    `  registerCompiledExpressionPlansInExpressionCache(expandCompactCompiledPlans(compactPlans, ${String(
      includeResolvedEvaluator,
    )}));`,
    '}',
    '',
...
export function generateAotLazyExpressionPreloadManifestModule(
  ...
  const code = [
    ...
    '];',
    '',
    `const compiledPlansByExpression = expandCompactCompiledPlans(compactPlans, ${String(
      includeResolvedEvaluator,
    )});`,
...
export function generateAotLazyExpressionsModule(
  ...
  const buildLazyModuleCode = (
    ...
    sections.push(
      ...
      `  registerCompiledExpressionPlansInExpressionCache(expandCompactCompiledPlans(compactPlans, ${String(includeResolvedEvaluator)}));`,
      '}',
      '',
    );
