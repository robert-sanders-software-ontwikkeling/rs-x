...
type KnownExpressionType =
  keyof (typeof ExpressionTreeBuilder.prototype)['expressionFactories'];
  ...
  public parseAst(
    ...
    try {
      ...
      if (e instanceof Error) {
        throw new ParserException(expressionSource, e.message);
      }

      throw new ParserException(expressionSource, String(e));
    } finally {
...
// Backward-compatible parser wrapper to avoid breaking downstream consumers.
@Injectable()
export class JsEspreeExpressionParser extends ExpressionTreeBuilder {
  constructor(
  ...
  public parse(expressionString: string): AbstractExpression {
    ...
    return this.parseAst(expressionAst, expressionString);
  }
}
