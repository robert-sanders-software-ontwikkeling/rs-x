...
describe('ExpressionManager', () => {
  ...
  it('The same expression is parsed only once', () => {
    ...
    const { instance: expression1 } = expressionManager
      .create(context1)
      .instance.create({ expressionString: 'a +1 * (2 + 3)' });
    const { instance: expression2 } = expressionManager
      .create(context2)
      .instance.create({ expressionString: 'a +1 * (2 + 3)' });
  ...
  it('dispose releases active expressions and cache references', () => {
    ...
    expressionManager.createAndGetInstance(context).create({ expressionString });
