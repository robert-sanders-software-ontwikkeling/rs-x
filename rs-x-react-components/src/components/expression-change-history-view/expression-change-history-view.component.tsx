...
function shortValue(value: unknown): string {
  ...
  if (valueType === 'string') {
    ...
    const clipped =
      stringValue.length > 80 ? `${stringValue.slice(0, 80)}…` : stringValue;
    return `'${clipped}'`;
  }
...
function pickHeaderChange(args: {
  ...
  rootExpression: IExpression | undefined;
...
function ValueDiff(props: {
  ...
  return (
    <div className={`valueDiff ${size === 'header' ? 'isHeader' : 'isRow'}`}>
      <span className={`valueDiffOld ${oldIsUnset ? 'isUnset' : ''}`}>
        {oldText}
      </span>
      <span className="valueDiffArrow">→</span>
      <span className={`valueDiffNew ${newIsUnset ? 'isUnset' : ''}`}>
        {newText}
      </span>
    </div>
  );
...
function getExpressionText(expression: IExpression): {
  ...
  const isTrigger =
    ...
    (parent?.type !== ExpressionType.Member || isLastChild);

  return {
    expressionString: expression.expressionString,
    type: isTrigger ? 'trigger' : 'derived',
  };
}
