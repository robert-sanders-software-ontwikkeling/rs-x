...
const COMPARISON_ROWS: IComparisonRow[] = [
  {
    ...
    'values': {
      'RS-X': 'Runtime-bound expression engine for fine-grained model reactivity',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Identifier (field, map key, array element, etc.) registered in the StateManager by an expression',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Binding the expression to a model registers all identifiers (fields/map keys/array indices) the expression depends on in the StateManager',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Per expression instance (isolated reactive graph; identifiers shared via reference counting in StateManager)',
      'React (Hooks)': 'Component tree (no implicit fine-grained graph)',
      'Vue 3 Reactivity': 'Shared runtime graph; scoped by component instance/effect scope',
      'Angular Signals': 'Shared runtime graph; scoped by injection/context + component lifecycle',
      'Svelte': 'Component scope (compiled updates) + store graphs',
      'SolidJS': 'Shared runtime graph; scoped by owner/root (component, createRoot)',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Yes. Each binding creates its own isolated reactive graph while sharing identifiers via reference counting.',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Disposing an expression removes its bindings. When no expressions reference an identifier, the StateManager removes reactive patches/proxies (reference-counted).',
      'React (Hooks)': 'N/A',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Yes. Create change tracker via ExpressionChangeTrackerManager (emits change history stacks) or use expression.changeHook for custom tracking',
      'React (Hooks)': 'No',
      'Vue 3 Reactivity': 'No',
      'Angular Signals': 'No',
      'Svelte': 'No',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Expressions parsed once into cached AST; multiple bindings reuse AST while creating isolated reactive graphs',
      'React (Hooks)': 'N/A',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Yes. Expressions are parsed independently and composable; cached AST reused; shared expressions evaluated once per dependency change',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Yes. Identifier resolution is pluggable. By default, identifiers resolve against the bound model and global scope (for example, Math or Date). But if you want, you can also resolve via a DOM ancestor path or custom context by implementing a custom identifier resolver.',
      'React (Hooks)': 'No',
      'Vue 3 Reactivity': 'No',
      'Angular Signals': 'No',
      'Svelte': 'No',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Generic reactive engine that supports UI change detection but is not limited to UI frameworks.',
      'React (Hooks)': 'Yes',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Changes are pushed directly to subscribers, so no scheduler is required by default. For batching, use IExpressionChangeTransactionManager to suspend() and continue() change emission.',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Yes. Using IExpressionChangeTransactionManager allows suspending change emission and resuming it, enabling atomic mutation batches with a single consolidated change notification.',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'No special async boundary in user code. Async and sync values are handled transparently, so you can mix them in the same expression (for example, a + b where a and b can each be async or sync) and updates propagate the same way.',
  ...
  },
  {
    ...
    'values': {
      'RS-X': 'Change tracker history plus expression hooks for introspection and custom tooling.',
