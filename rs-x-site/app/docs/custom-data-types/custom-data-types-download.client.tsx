...
import { buildZipFromTextFiles } from '@rs-x/react-components';

// ─── project files ─────────────────────────────────────────────────────────

const packageJson = JSON.stringify(
  {
    name: 'rs-x-custom-data-types',
    private: true,
    version: '0.1.0',
    type: 'module',
    scripts: {
...
const readme = `# rs-x Custom Data Types Example

Demonstrates how to teach rs-x to observe a custom class (\`TextDocument\`)
by implementing the full set of contracts:

1. \`AbstractObserver\` — wraps the object in a Proxy, emits \`IPropertyChange\`
2. Index observer — watches a single \`{ pageIndex, lineIndex }\` cell
3. \`IIndexValueAccessor\` — reads/writes individual cells
4. Observer proxy pair factories — wires object- and index-level observation
5. \`KeyedInstanceFactory\` — shared, reference-counted observer instances
6. DI registration via \`ContainerModule\` + \`overrideMultiInjectServices\`
7. \`IIdentifierOwnerResolver\` — enables \`rsx('doc[cell]')(model)\` expression binding
...
const mainTs = `/**
 * rs-x custom data type demo.
 *
 * Shows how to use TextDocument (a custom class with compound index) with
 * the full rs-x observation stack, including rsx() expression binding.
 */
...
class TextDocument {
  ...
  public setLine(index: ITextDocumentIndex, text: string): void {
    ...
    if (!page) { page = new Map(); this._pages.set(index.pageIndex, page); }
...
const MyTokens = {
  ...
  TextDocumentIndexObserverProxyPairFactory: Symbol('TextDocumentIndexObserverProxyPairFactory'),
...
class TextDocumentObserver extends AbstractObserver<TextDocument> {
  ...
  public get(doc: TextDocument, property: PropertyKey, receiver: unknown): unknown {
    if (property === 'setLine') {
...
class TextDocumentIndexObserver extends AbstractObserver<TextDocument, string, ITextDocumentIndex> {
  ...
  constructor(
    ...
    super(owner, _obs.target, _obs.target.getLine(index), new ReplaySubject(), index);
  ...
  private readonly _onChange = (change: IPropertyChange) => {
    ...
    if (idx?.lineIndex === this.id?.lineIndex && idx?.pageIndex === this.id?.pageIndex) {
...
@Injectable()
class TextDocumentObserverManager extends KeyedInstanceFactory<TextDocument, TextDocument, TextDocumentObserver> {
  constructor(
    @Inject(RsXCoreInjectionTokens.IProxyRegistry)
    private readonly _proxyRegister: IProxyRegistry,
  ) { super(); }

  public override getId(doc: TextDocument): TextDocument { return doc; }
  protected override createId(doc: TextDocument): TextDocument { return doc; }
...
class IndexForDocumentObserverManager extends KeyedInstanceFactory<number, IIndexObserverInfo<ITextDocumentIndex>, TextDocumentIndexObserver> {
  constructor(
  ...
  public override getId(info: IIndexObserverInfo<ITextDocumentIndex>): number { return this._id(info); }
  protected override createId(info: IIndexObserverInfo<ITextDocumentIndex>): number { return this._id(info); }
  private _id({ index: { pageIndex, lineIndex } }: IIndexObserverInfo<ITextDocumentIndex>): number {
    return ((pageIndex + lineIndex) * (pageIndex + lineIndex + 1)) / 2 + lineIndex;
  }
  protected override createInstance(info: IIndexObserverInfo<ITextDocumentIndex>, id: number): TextDocumentIndexObserver {
    const obs = this._docMgr.create(this._doc).instance;
    return new TextDocumentIndexObserver(
      { canDispose: () => this.getReferenceCount(id) === 1, release: () => { obs.dispose(); this.release(id); } },
      obs,
      info.index,
    );
  }
  protected override onReleased(): void { if (this.isEmpty) this._releaseOwner(); }
}

@Injectable()
class TextDocumentIndexObserverManager extends KeyedInstanceFactory<TextDocument, TextDocument, IndexForDocumentObserverManager> {
  constructor(
  ...
  public override getId(doc: TextDocument): TextDocument { return doc; }
  protected override createId(doc: TextDocument): TextDocument { return doc; }
  protected override createInstance(doc: TextDocument): IndexForDocumentObserverManager {
    return new IndexForDocumentObserverManager(doc, this._docMgr, () => this.release(doc));
  }
  protected override releaseInstance(mgr: IndexForDocumentObserverManager): void { mgr.dispose(); }
}
...
class TextDocumentObserverProxyPairFactory implements IObjectObserverProxyPairFactory {
  ...
  public applies(object: unknown): boolean { return object instanceof TextDocument; }
  public create(_: IDisposableOwner, proxyTarget: IProxyTarget<TextDocument>): IObserverProxyPair<TextDocument> {
    const observer = this._mgr.create(proxyTarget.target).instance;
    return { observer, proxy: observer.target as TextDocument, proxyTarget: proxyTarget.target };
  }
}

@Injectable()
class TextDocumentIndexObserverProxyPairFactory extends IndexObserverProxyPairFactory<TextDocument, unknown> {
  constructor(
    ...
    super(objectObserverManager, Type.cast(indexObserverManager), errorLog, guidFactory, indexValueAccessor, proxyRegister, valueMetadata);
  }
  public applies(object: unknown, propertyInfo: IPropertyInfo): boolean {
    const idx = propertyInfo.index as ITextDocumentIndex;
    return object instanceof TextDocument && typeof idx?.lineIndex === 'number' && typeof idx?.pageIndex === 'number';
  }
}
...
@Injectable()
class TextDocumentIndexAccessor implements IIndexValueAccessor<TextDocument, ITextDocumentIndex> {
  public readonly priority = 200;
  public applies(object: unknown, index: ITextDocumentIndex): boolean {
    return object instanceof TextDocument && typeof index?.pageIndex === 'number' && typeof index?.lineIndex === 'number';
  }
  public hasValue(doc: TextDocument, index: ITextDocumentIndex): boolean { return doc.getLine(index) !== undefined; }
  public getValue(doc: TextDocument, index: ITextDocumentIndex): string | undefined { return doc.getLine(index); }
  public getResolvedValue(doc: TextDocument, index: ITextDocumentIndex): string | undefined { return doc.getLine(index); }
  public setValue(doc: TextDocument, index: ITextDocumentIndex, value: string): void { doc.setLine(index, value); }
  public getIndexes(): IterableIterator<ITextDocumentIndex> { return [].values(); }
}
...
class TextDocumentIndexOwnerResolver implements IIdentifierOwnerResolver {
  public resolve(index: unknown, context: unknown): object | null {
    ...
    if (typeof idx?.pageIndex !== 'number' || typeof idx?.lineIndex !== 'number') return null;
    return context;
  }
}
...
const MyModule = new ContainerModule((options) => {
  options.bind(MyTokens.TextDocumentObserverManager).to(TextDocumentObserverManager).inSingletonScope();
  options.bind(MyTokens.TextDocumentIndexObserverManager).to(TextDocumentIndexObserverManager).inSingletonScope();

  overrideMultiInjectServices(options, RsXCoreInjectionTokens.IIndexValueAccessorList, [
    { target: TextDocumentIndexAccessor, token: MyTokens.TextDocumentIndexAccessor },
    ...defaultIndexValueAccessorList,
  ]);
  overrideMultiInjectServices(options, RsXStateManagerInjectionTokens.IObjectObserverProxyPairFactoryList, [
    { target: TextDocumentObserverProxyPairFactory, token: MyTokens.TextDocumentObserverProxyPairFactory },
    ...defaultObjectObserverProxyPairFactoryList,
  ]);
  overrideMultiInjectServices(options, RsXStateManagerInjectionTokens.IPropertyObserverProxyPairFactoryList, [
    { target: TextDocumentIndexObserverProxyPairFactory, token: MyTokens.TextDocumentIndexObserverProxyPairFactory },
    ...defaultPropertyObserverProxyPairFactoryList,
  ]);
...
const proxyRegistry = InjectionContainer.get<IProxyRegistry>(RsXCoreInjectionTokens.IProxyRegistry);
const emptyFn = () => {};
...
async function testInitialRead(): Promise<void> {
  ...
  const model = { doc, cell: { pageIndex: 1, lineIndex: 0 } as ITextDocumentIndex };
...
async function testCellAddressChange(): Promise<void> {
  ...
  const model = { doc, cell: { pageIndex: 0, lineIndex: 0 } as ITextDocumentIndex };
...
function downloadCustomDataTypesProject(): void {
  ...
  anchor.href = url;
  anchor.download = 'rs-x-custom-data-types.zip';
  anchor.click();

  window.setTimeout(() => URL.revokeObjectURL(url), 0);
}

export function CustomDataTypesDownloadButton() {
  return (
    <button
      type="button"
      className="btn btnGhost btnSm"
      onClick={downloadCustomDataTypesProject}
    >
      Download project
    </button>
  );
