...
const MEMORY_TABS: MemoryTab[] = [
  ...
  {
    value: 'bind-same',
    label: 'Bind same expression',
    prefix: 'Bind same expression',
  },
...
export function MemoryUsageTabs({ rows }: MemoryUsageTabsProps) {
  ...
  const activeTab =
    MEMORY_TABS.find((t) => t.value === selected) ?? MEMORY_TABS[0];
  const filteredRows = rows.filter((r) =>
    r.scenario.startsWith(activeTab.prefix),
  );

  const chartRows = filteredRows.map((r) => ({
    label: r.scenario
      .replace(`${activeTab.prefix} `, '')
      .replace(/[()]/g, '')
      .trim(),
