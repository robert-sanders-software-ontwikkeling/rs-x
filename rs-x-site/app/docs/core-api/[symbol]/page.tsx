...
const MODULE_DETAILS: Record<string, string> = {
  ...
  'value-metadata':
    'Value classification module used by runtime services. It finds the first metadata provider in `IValueMetadataList` (sorted by priority) whose `applies(value)` returns true, then uses that provider to evaluate `isAsync(value)` and `needsProxy(value)`.',
  'wait-for-event':
...
const MEMBER_DESCRIPTION_OVERRIDES: Record<string, Record<string, string>> = {
  ...
  ValueMetadata: {
    ...
    isAsync:
      'Finds the matching metadata provider for the value and returns whether that value type is treated as async.',
    needsProxy:
      'Finds the matching metadata provider for the value and returns whether that value type should be proxied.',
    applies:
      'Returns true when at least one metadata provider in the injected list can handle the value.',
    getValueMetadata:
      'Selects the first matching metadata provider (highest priority first). Throws `UnsupportedException` when none match.',
  },
  ArrayMetadata: {
