    ...
    <DocsPageTemplate>
      ...
      <Card header="Overview">
        <p className="cardText">
          <span className="codeInline">WaitForEvent</span> helps you trigger an
          action and then wait for emissions from an Observable event property
          on a target object.
        </p>
        <p className="cardText">
          Configure <span className="codeInline">count</span> to wait for one or
          more emissions, <span className="codeInline">timeout</span> to cap how
          long it waits, and{' '}
          <span className="codeInline">ignoreInitialValue</span> when the first
          emission should be skipped.
        </p>
        <p className="cardText">
          The <span className="codeInline">wait(trigger)</span> call subscribes,
          runs the trigger (sync/Promise/Observable), collects emitted values,
          and resolves with the value (or values). If timeout is reached first,
          it resolves <span className="codeInline">null</span>.
        </p>
      </Card>
