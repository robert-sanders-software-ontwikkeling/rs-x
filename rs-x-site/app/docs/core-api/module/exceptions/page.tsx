    ...
    <DocsPageTemplate>
      ...
      <Card header="Current exceptions implementation">
        <p className="cardText">
          This module contains typed error classes used across rs-x core. Most
          exceptions extend <span className="codeInline">CustomError</span> so
          error names remain stable and easier to handle in logs/tests.
        </p>
        <p className="cardText">
          It also includes the static{' '}
          <span className="codeInline">Assertion</span> helper for common guard
          checks (predicate, function type, null/empty). These guards throw
          specific exception types so failures are explicit.
        </p>
      </Card>
