...
function nextInstrumentationFilePath(projectRoot, entryFile) {
  // instrumentation.ts must live at the project root (or /src if that exists)
  const srcDir = path.join(projectRoot, 'src');
  const ext = path.extname(entryFile).toLowerCase();
  const useTs = ext === '.ts' || ext === '.tsx';
  const fileName = useTs ? 'instrumentation.ts' : 'instrumentation.js';
  const dir = fs.existsSync(srcDir) ? srcDir : projectRoot;
  return path.join(dir, fileName);
}

function ensureNextInstrumentationFile(instrumentationFile, dryRun) {
  if (fs.existsSync(instrumentationFile)) {
    const existing = fs.readFileSync(instrumentationFile, 'utf8');
    if (existing.includes('RsXExpressionParserModule')) {
      logInfo(
        `Next instrumentation already configured: ${instrumentationFile}`,
      );
      return;
    }
    logWarn(
      `Instrumentation file exists but does not load RS-X: ${instrumentationFile}`,
    );
    logInfo(
      'Add the following to your instrumentation register() function:\n' +
        "  import { InjectionContainer } from '@rs-x/core';\n" +
        "  import { RsXExpressionParserModule } from '@rs-x/expression-parser';\n" +
        '  await InjectionContainer.load(RsXExpressionParserModule);',
    );
  ...
  const useTs = instrumentationFile.endsWith('.ts');
  const content = useTs
    ? `import { InjectionContainer } from '@rs-x/core';
import { RsXExpressionParserModule } from '@rs-x/expression-parser';

// Generated by rsx init — loads RS-X DI on the server before any page module is imported
...
`;
  ...
  if (dryRun) {
    logInfo(`[dry-run] create ${instrumentationFile}`);
    return;
  }

  fs.writeFileSync(instrumentationFile, content, 'utf8');
  logOk(`Created ${instrumentationFile}`);
}
...
function wrapAppEntry(source) {
  ...
  return `import { InjectionContainer } from '@rs-x/core';
import { RsXExpressionParserModule } from '@rs-x/expression-parser';
${importSection}
...
function patchEntryFileForRsx(entryFile, bootstrapFile, context, dryRun) {
  ...
  if (context === 'react' || context === 'vuejs') {
    if (
      original.includes('rsxBootstrap') ||
      original.includes('InjectionContainer')
    ) {
      logInfo(`Entry already wired for RS-X: ${entryFile}`);
      return true;
    }

    const updated = wrapAppEntry(original);
    if (!updated) {
...
function runInit(flags) {
  ...
  if (!entryFile) {
    ...
    const instrumentationFile = nextInstrumentationFilePath(
      projectRoot,
      entryFile,
    );
    ...
    ensureNextInstrumentationFile(instrumentationFile, dryRun);
...
module.exports = {
  // Required for Next.js 13.x; no-op in Next.js 14+
...
`;
  ...
  if (original.includes('__rsxWebpackLoaderPath')) {
    ...
    if (!original.includes('instrumentationHook')) {
      logWarn(
        'If you are on Next.js 13.x, add `experimental: { instrumentationHook: true }` to your next.config.js to enable instrumentation.ts.',
      );
    }
    return;
  }
