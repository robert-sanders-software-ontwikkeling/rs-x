...
function resolveLazyAotFilePaths(
  ...
  return {
    ...
    payloadFilePath: path.join(dir, `rsx-aot-lazy.generated${ext}`),
  };
}
...
function runRsxAotLazyGeneration({
  ...
  const ungroupedSpecifier = toImportSpecifier(
    manifestOutputFile,
    payloadOutputFile,
  );
  const [ungroupedDir, ungroupedFile] = splitDynamicImport(ungroupedSpecifier);
  const manifestLines = [
    'import {',
    ...
    `    ungroupedModulePromise = importLazyModule('${ungroupedDir}' + '${ungroupedFile}').then((lazyModule) => {`,
  ...
  for (const [groupName] of groupEntries) {
    ...
    const groupSpecifier = toImportSpecifier(manifestOutputFile, groupFilePath);
    const [groupDir, groupFile] = splitDynamicImport(groupSpecifier);
    ...
    manifestLines.push(
      ...
      `    groupModulePromise_${safeName} = importLazyModule('${groupDir}' + '${groupFile}').then((lazyModule) => {`,
      '      lazyModule.registerRsxAotLazyExpressions?.();',
      '    });',
      '  }',
