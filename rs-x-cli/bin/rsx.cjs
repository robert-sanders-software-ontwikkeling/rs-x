...
function runRsxAotLazyGeneration({
  ...
  if (
    !compilerModule ||
    typeof compilerModule.generateAotLazyExpressionPreloadManifestModule !==
      'function' ||
  ...
  const manifestGenerated =
    compilerModule.generateAotLazyExpressionPreloadManifestModule(program, {
      includeResolvedEvaluator,
    });
  ...
  const manifestContent = `${header}${manifestGenerated.code}`;
  const payloadContent = `${header}${payloadGenerated.code}`;

  if (dryRun) {
    logInfo(
      `[dry-run] generate RS-X lazy AOT manifest (${manifestGenerated.expressions.length} expressions): ${manifestOutputFile}`,
    );
    logInfo(
  ...
  logOk(
    `Generated RS-X lazy AOT manifest (${manifestGenerated.expressions.length} expressions): ${path.relative(projectRoot, manifestOutputFile)}`,
  );
