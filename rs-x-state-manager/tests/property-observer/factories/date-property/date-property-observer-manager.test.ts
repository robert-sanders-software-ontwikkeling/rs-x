...
describe('IDatePropertyObserverManager tests', () => {
  ...
  it('will release observers when all references have nee disposed', async () => {
    const date = new Date(2021, 1, 3);
    const observerForDatePropertyManager =
      datePropertyObserverManager.create(date);

    const observer1 = observerForDatePropertyManager.instance.create({
      index: 'year',
    }).instance;
    const observer2 = observerForDatePropertyManager.instance.create({
      index: 'year',
    }).instance;
