...
class ShouldWatchIndexPredicateForContextManager extends SingletonFactory<
  unknown,
  unknown,
  ShouldWatchIndex
> {
  ...
  protected createInstance(indexOnContext: unknown): ShouldWatchIndex {
    return (index, indexTarget) =>
      index === indexOnContext && indexTarget === this._context;
  }
...
class ShouldWatchIndexPredicateManager extends SingletonFactory<
  unknown,
  unknown,
  ShouldWatchIndexPredicateForContextManager
> {
  constructor() {
    super();
  }
  ...
  protected createInstance(
    ...
    return new ShouldWatchIndexPredicateForContextManager(context, () =>
      this.release(id),
    );
  }
